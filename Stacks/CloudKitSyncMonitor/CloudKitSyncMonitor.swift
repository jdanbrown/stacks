// Based on https://github.com/ggruen/CloudKitSyncMonitor/blob/1.1.1/Sources/CloudKitSyncMonitor/SyncMonitor.swift
//  - Originally created by Grant Grueninger on 9/23/20
...
// leave it alone. Every once in a while, however, it will hit an error that makes it stop syncing. This is what you really want to detect,
// because, since iCloud is the "source of truth" for your `NSPersistentCloudKitContainer` data, a sync failure can mean data loss.
//
// Here are the basics:
//
...
// values that let you get even more granular, e.g. to find whether each type of event is in progress, succeeded, or failed, the start and end
// time of the event, and any error reported if the event failed.
//
...
public class CloudKitSyncMonitor: ObservableObject {
  ...
  public var syncStateSummary: SyncSummaryStatus {
    ...
    guard case .available = iCloudAccountStatus else {
    ...
    if case .inProgress = importState {
      return .inProgress
    }
    if case .inProgress = exportState {
      return .inProgress
    }
  ...
  public enum SyncSummaryStatus {
    ...
    public var symbolName: String {
      switch self {
        ...
        case .notSyncing:          return "xmark.icloud"
        case .notStarted:          return "bolt.horizontal.icloud"
        case .inProgress:          return "arrow.clockwise.icloud"
        case .succeeded:           return "icloud"
  ...
  // Returns `true` if there's no reason that we know of why sync shouldn't be working, i.e.
  //  - The user's iCloud account status is "available"
  ...
  //
  // Specific Status Properties
  //

  @Published public private(set) var setupState:  SyncState = .notStarted // NSPersistentCloudKitContainer's setup event
  @Published public private(set) var importState: SyncState = .notStarted // NSPersistentCloudKitContainer's import event
  @Published public private(set) var exportState: SyncState = .notStarted // NSPersistentCloudKitContainer's export event

  // Is the network available?
  //  - true if the network is available in any capacity (Wi-Fi, Ethernet, cellular, carrier pidgeon, etc)
  //  - We just care if we can reach iCloud
  ...
  //  - Automatically updated if they change it
  ...
  //  - Helpful in diagnosing "notSyncing" issues or other "partial error"s from which CloudKit thinks it recovered, but didn't really
  ...
  //
  // Processing NSPersistentCloudKitContainer events
  //
  ...
  internal func setProperties(from event: SyncEvent) {
    ...
    //  - If it has an endDate, it means the event finished
  ...
  public enum SyncState {
    ...
    case succeeded(started: Date, ended: Date)             // The last sync of this type finished with succeeded=true
    case failed(started: Date, ended: Date, error: Error?) // The last sync of this type finished with succeeded=false

    var succeeded: Bool { if case .succeeded = self { return true } else { return false } }
